...
class StrategySelector:
    ...
    def _calculate_scores(self, X_tensor, strategy):
        ...
        # Confidence-based strategy: Select samples with low aleatoric uncertainty (high confidence)
        elif strategy == 'confidence':
            n_alea = norm(alea)
            return 1.0 - n_alea
        # Uncertainty-based strategy: Select samples with high entropy (total uncertainty) (ALM)
        elif strategy == 'entropy':
            return epi + alea
        # Epistemic-only strategy: Select samples with high epistemic uncertainty (QBC)
        elif strategy == 'epistemic':
            return epi
        # Aleatoric-only strategy: Select samples with high aleatoric uncertainty (noisy samples) (ale)
